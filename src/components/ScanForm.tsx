...
/** Pull UTM params from the current URL so scans can be attributed to a campaign. */
...
export function ScanForm({
  ...
  useEffect(() => {
    analytics.scanFormViewed({ tier, mode });
    // Fire once on mount — the initial tier/mode the visitor lands on.
  ...
  const startCheckout = async (domain: string, quantity: number) => {
    ...
    setLoading(true);
    setError("");
    analytics.checkoutQuantitySelected({
      tier,
      mode,
      quantity,
      cents: priceCents(tier, mode, quantity),
    });
    ...
    try {
      ...
      if (hasCredit || useFreeFlow) {
        const res = await fetch("/api/scan/start", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ domain, tier, mode, email: verifiedFreeEmail || undefined, utm: readUtm() }),
        });
        if (res.ok) {
          const { scanId, source } = await res.json();
          analytics.scanStarted({ tier, mode, source: source === "first-free" ? "first-free" : "wallet" });
      ...
      analytics.checkoutOpened({ tier, mode });
      setLoading(false);
    } catch (err) {
      ...
      >
            ...
            {TIER_ORDER.map((id, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    setTier(id);
                    analytics.tierSelected({ tier: id, mode });
                  }}
        ...
        >
          ...
          <button
            type="button"
            onClick={() => setMode((m) => {
              const next = m === "max" ? "standard" : "max";
              analytics.maxModeToggled({ enabled: next === "max" });
              return next;
            })}
