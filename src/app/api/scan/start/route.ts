...
export async function POST(req: NextRequest) {
  ...
  // Acquisition attribution: referrer from the request header, UTM from the
  // client (it owns the landing-page query string). Capped to keep rows small.
  const cap = (v: unknown): string | null =>
    typeof v === "string" && v.trim() ? v.trim().slice(0, 255) : null;
  const attribution = {
    referrer: cap(req.headers.get("referer")),
    utmSource: cap(body.utm?.source),
    utmMedium: cap(body.utm?.medium),
    utmCampaign: cap(body.utm?.campaign),
  };
  ...
  if (walletId) {
    try {
      ...
      if (balances[key] > 0) {
        const { id } = await createFreeBypassScan(domain, { tier, mode, ipHash, userAgentHash, attribution });
