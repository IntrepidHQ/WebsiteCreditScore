...
export function LiveReport({ scanId, domain, initialResult, depth = "aerial" }: Props) {
        ...
        if (event.type === "search" && event.query) {
          ...
          if (event.report) {
            ...
            analytics.scanCompleted({
              grade: event.report.overall.grade,
              score: event.report.overall.score,
          ...
          analytics.scanFailed({ reason: event.error ?? "unknown" });
          es.close();
        }
      } catch {
        // ignore parse errors
