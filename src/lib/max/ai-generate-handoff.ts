...
export async function generateMaxHandoffWithClaude(
  ...
  const userContent = [
    ...
    "8) After the markdown document body, append a line containing only ---WCS_CODING_PROMPT--- surrounded by blank lines.",
    "9) After that delimiter, output only the paste-ready prompt for the coding agent (second person, imperative plain text — no markdown fences, no headings).",
    "",
    "Audit JSON:",
